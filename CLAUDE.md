...
- **Sampling is tuned for *distinct* output, and `repeat_penalty` matters far more than `temperature`.** The buffer dedups, so a round that adds nothing new is wasted (and backs off). Measured on `qwen3:8b`, 60 words requested: its own parameters (`temperature 0.6`, `repeat_penalty 1`) gave 30 distinct / 75 lines (60% dupes); `temperature 1.0` alone gave *64%* dupes; adding `repeat_penalty 1.2` gave 2%. Hence the defaults (1.0 / 1.2). Do not raise the penalty much higher — at 1.5 sentence yield halved, since it suppresses the common words sentences are made of. Unlike `think`, these need no capability check: Ollama accepts them for every completion model.
